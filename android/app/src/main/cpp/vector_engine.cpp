...
public:
               ...
               const std::vector<float>& emb, DistFn dist)
    {
        ...
        if (store.count(id)) {
            // Handle updates by removing old entry first
            store.erase(id);
            bf.remove(id);
            hnsw.remove(id);
...
#define EXPORT __attribute__((visibility("default"))) __attribute__((used))
    ...
    EXPORT void* vector_db_create(int dims) {
        return new VectorDB(dims);
    }

    EXPORT void vector_db_destroy(void* db_ptr) {
        if (db_ptr) {
            delete static_cast<VectorDB*>(db_ptr);
        }
    }

    EXPORT void vector_db_insert(void* db_ptr, int id, const float* emb, int emb_len) {
        if (!db_ptr || !emb) return;
        VectorDB* db = static_cast<VectorDB*>(db_ptr);
        if (emb_len != db->dims) return;
        
        std::vector<float> emb_vec(emb, emb + emb_len);
        db->insert(id, "", "", emb_vec, getDistFn("cosine"));
    }
    
    EXPORT void vector_db_remove(void* db_ptr, int id) {
        if (!db_ptr) return;
        VectorDB* db = static_cast<VectorDB*>(db_ptr);
        db->remove(id);
    }
    ...
    EXPORT SearchResultList vector_db_search(void* db_ptr, const float* query, int query_len, int k) {
        SearchResultList list = {nullptr, 0};
        if (!db_ptr || !query) return list;

        VectorDB* db = static_cast<VectorDB*>(db_ptr);
        if (query_len != db->dims) return list;
        
        std::vector<float> q_vec(query, query + query_len);

        auto out = db->search(q_vec, k, "cosine", "hnsw");
        
        list.count = out.hits.size();
        if (list.count > 0) {
            list.results = (SearchResult*)malloc(list.count * sizeof(SearchResult));
            for (int i = 0; i < list.count; i++) {
                list.results[i].id = out.hits[i].id;
                list.results[i].distance = out.hits[i].dist;
            }
        }
        return list;
    }

    EXPORT void vector_db_free_results(SearchResultList list) {
        if (list.results) {
            free(list.results);
        }
    }
